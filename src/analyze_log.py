...
# statistics module do python https://www.w3schools.com/python/module_statistics.asp
...
def analyze_log(path_to_file):
    ...
    try:
        with open(path_to_file) as text:
            ...
            joao_not_consumed = items.difference(set([
                item[1] for item in file
                if item[0] == "joao"
            ]))
            
            days = set([
                item[2] for item in file
            ])
            
            joao_not_days = days.difference(set([
                item[2] for item in file
                if item[0] == "arnaldo"
            ]))
